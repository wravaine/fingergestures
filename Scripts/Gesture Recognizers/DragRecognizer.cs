...
public class DragRecognizer : ContinuousGestureRecognizer<DragGesture>
{
    ...
    protected override bool CanBegin( DragGesture gesture, FingerGestures.IFingerList touches )
    {
        ...
        float dist = touches.GetAverageDistanceFromStart();
        float distDIP = FingerGestures.ToDensityIndependentPixel( dist );
        //Debug.Log( "drag: dist=" + dist + " distDIP=" + distDIP + "/" + MoveTolerance );

        // must have moved beyond move tolerance threshold
        if( distDIP < MoveTolerance )   //if( FingerGestures.GetDeviceIndependentPixelDistance( touches.GetAverageDistanceFromStart() ) < MoveTolerance )
            return false;
