...
public class FingerGestures : MonoBehaviour
{
    ...
    #region Device Independent Pixel Unit

    public bool useDeviceIndependentPixelUnit = true;

    [SerializeField]
    int referencePixelsPerInch = 160;           // reference pixel density used to configure the various recognizers

    /// <summary>
    /// Density-Independent Pixel Scale
    ///     dipScale * physical_pixel = density_independent_pixel
    /// </summary>
    float dipScale = 1.0f;

    /// <summary>
    /// Convert physical pixel value to density-independent pixel value (DIP)
    /// </summary>
    /// <param name="physicalPixelDistance">The raw pixel value</param>
    public static float ToDensityIndependentPixel( float physicalPixelDistance )
    {
        return instance.useDeviceIndependentPixelUnit ? DensityIndependentPixelScale * physicalPixelDistance : physicalPixelDistance;
    }

    /// <summary>
    /// Reference DPI/PPI (DPI = dots-per-inch, PPI = pixels-per-inch - same thing)
    ///   This is the reference DPI value that the various density-independent pixel properties on the gesture recognizers are based off
    /// </summary>
    public static int ReferencePixelsPerInch
    {
        get { return instance.referencePixelsPerInch; }

        set
        {
            instance.referencePixelsPerInch = value;
            instance.UpdateDIP();
        }
    }

    public static float DensityIndependentPixelScale
    {
        get { return instance.dipScale; }
    }

    void UpdateDIP()
    {
        if( Screen.dpi > 0 && ReferencePixelsPerInch > 0 )
        {
            dipScale = ReferencePixelsPerInch / Screen.dpi;
           //Debug.Log( "DIP Scale set to " + dipScale + " (DPI = " + Screen.dpi + ")" );
        }
        else
        {
            // cannot retrieve physical screen pixel density from system
            // use a DIP scale of 1.0 to make 1 DIP = 1 physical pixel
            dipScale = 1.0f;
        }
    }
